...
async function captureDownload(downloadItem, suggest) {
    ...
    if (Configs.integration && shouldCapture(downloadItem)) {
        ...
        // Phase 3: Native Messaging takeover pipeline
        try {
            if (nativeClient.isSupportedUrl(downloadItem.url)) {
                let cookies = await getCookies(downloadItem);
                let headers = [];
                if (cookies && cookies.length > 0) {
                    headers.push("Cookie: " + cookies.join(";"));
                }
                const result = await nativeClient.addDownload(downloadItem, headers);
                // Step 3 Contract: ONLY cancel browser download AFTER receiving real 16-hex GID!
                if (result && result.status === "Success" && result.gid && result.gid.length === 16) {
                    await chrome.downloads.cancel(downloadItem.id);
                    if (chrome.runtime.lastError) chrome.runtime.lastError = null;
                    return;
                } else {
                    console.warn("[AriaUI Native] Download not accepted by native engine:", result);
                }
            } else {
                console.info("[AriaUI Native] URL scheme not supported for native takeover:", downloadItem.url);
            }
        } catch (nativeErr) {
            console.warn("[AriaUI Native] Native takeover failed:", nativeErr);
        }

        // Native takeover did not succeed or unsupported: smoothly hand back to browser by leaving downloadItem untouched.
        // Do NOT cancel browser download; do NOT provide HTTP/TCP RPC fallback.
        return;
    }
}
