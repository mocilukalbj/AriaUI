...
日期 / 应用 commit / aria2 commit / 构建模式：
  2026-09-05 14:18:07 UTC / git-master / aria2 1.37.0 / .NET 10.0 (Release/Debug)
...
|---|---|---|---|---|---|---|---|
| 空任务 Session 启动 | 30 | 240.31 | 260.34 | 323.14 | 338.66 | 268.72 | < 500ms 达成 |
| 预置 10 任务 Session 恢复 | 30 | 242.05 | 254.63 | 308.12 | 340.57 | 263.67 | < 500ms 达成 |
...
|---|---|---|---|---|---|---|---|
| 第 1 轮 | aria2.getGlobalStat | 1000 | 0.30 | 0.90 | 1.23 | 5.78 | 0.92 |
| 第 2 轮 | aria2.getGlobalStat | 1000 | 0.38 | 0.81 | 1.07 | 8.10 | 0.85 |
| 第 3 轮 | aria2.getGlobalStat | 1000 | 0.12 | 0.79 | 1.03 | 6.31 | 0.77 |

> **三轮 P95 中位门禁基线值**：`1.07 ms`（阶段 4 候选原生实现上限阈值为：`1.18 ms`）。
...
| aria2c 工作集内存 (RSS) | 29.98 MiB | 进程物理内存常驻集 |
| aria2c 私有提交内存 (Private Memory) | 17.96 MiB | 专用内存分配 |
| aria2c 线程数 | 1 | 活动线程数 |

## 4. 结论与阶段退出判定

1. 真实 RPC 冷启动时间均值在 268.7ms 级别，远优于 500ms 试验目标，空任务与恢复任务无异常延迟。
2. 单命令 RPC 往返 P95 稳定在 1.07ms，为阶段 1 原生探针和阶段 4 原生实现确定了严格的性能对比基准。
