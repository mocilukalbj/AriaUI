...
日期 / 应用 commit / aria2 commit / 构建模式：
  2026-09-05 14:54:24 UTC / git-master / aria2 1.37.0 (via libaria2_bridge) / .NET 10.0 (NativeAriaEngineHost)
...
| 空任务 Session 启动 | **Native 引擎** | 30 | 3.78 | 4.2 | 6.56 | 14.42 | 4.63 | **< 500ms 达成** |
| 预置 10 任务恢复 | RPC 基线 | 30 | 241.44 | 261.20 | 318.51 | 326.93 | 268.90 | < 500ms 达成 |
| 预置 10 任务恢复 | **Native 引擎** | 30 | 4.26 | 5.25 | 6.45 | 15.03 | 5.63 | **< 500ms 达成** |

## 2. 常用命令延迟对比 (Command Latency)

| 轮次 | 命令类型 | 调用次数 | Min (ms) | P50 (ms) | P95 (ms) | Max (ms) | Avg (ms) |
|---|---|---|---|---|---|---|---|
| 第 1 轮 | GetGlobalOptionAsync (Native Engine) | 1000 | 0.011 | 0.035 | 0.109 | 0.494 | 0.050 |
| 第 2 轮 | GetGlobalOptionAsync (Native Engine) | 1000 | 0.066 | 0.097 | 0.196 | 34.086 | 0.155 |
| 第 3 轮 | GetGlobalOptionAsync (Native Engine) | 1000 | 0.658 | 1.373 | 2.259 | 11.438 | 1.481 |

> **RPC 基线三轮 P95 中位数**：`1.07 ms`
> **Native 引擎三轮 P95 中位数**：`0.196 ms`
...
|---|---|---|---|
| 空闲 CPU 占用率 | 0.00% | 0.24% | 1 秒空闲采样 (< 0.5% 预算达成) |
| 物理常驻内存 (RSS) | 29.96 MiB | 101.64 MiB | 包含 .NET 运行时与 Native 引擎 |
| 私有提交内存 (Private Memory) | 17.96 MiB | 264.71 MiB | 进程私有内存 |
...
2. **命令延迟达标**：Native 命令往返三轮 P95 中位数为 `0.196 ms`，远低于 RPC 基线门禁上限 `1.18 ms`，P95 延迟完全符合 §4 要求。
