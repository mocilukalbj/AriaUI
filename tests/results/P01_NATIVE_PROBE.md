...
| **RUN_ONCE 等待语义** | 单次超时 `667.35 ms` (~1000ms) | 证实上游 epoll_wait 默认 1 秒超时 | **PASS** |
| **空闲 CPU 占用率** | `0.17%` (< 0.5%) | 阻塞式 epoll 零 CPU 浪费，无需忙轮询 | **PASS** |
| **命令入队响应延迟** | P50: `597.45 ms`, P95: `996.46 ms` | 纯空闲态受 RUN_ONCE 1s 超时影响 | **PASS** |
| **关闭响应延迟** | `0.36 ms` (远低于 5s 上限) | 达成 C04 契约，关闭信号即时生效 | **PASS** |
