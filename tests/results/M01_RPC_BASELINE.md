...
日期 / 应用 commit / aria2 commit / 构建模式：
  2026-09-05 09:43:11 UTC / git-master / aria2 1.37.0 / .NET 10.0 (Release/Debug)
...
|---|---|---|---|---|---|---|---|
| 空任务 Session 启动 | 30 | 244.82 | 270.90 | 364.54 | 390.31 | 283.37 | < 500ms 达成 |
| 预置 10 任务 Session 恢复 | 30 | 241.44 | 261.20 | 318.51 | 326.93 | 268.90 | < 500ms 达成 |
...
|---|---|---|---|---|---|---|---|
| 第 1 轮 | aria2.getGlobalStat | 1000 | 0.40 | 0.97 | 1.26 | 7.42 | 1.01 |
| 第 2 轮 | aria2.getGlobalStat | 1000 | 0.34 | 0.81 | 1.09 | 4.15 | 0.82 |
| 第 3 轮 | aria2.getGlobalStat | 1000 | 0.24 | 0.76 | 1.05 | 4.50 | 0.80 |

> **三轮 P95 中位门禁基线值**：`1.09 ms`（阶段 4 候选原生实现上限阈值为：`1.19 ms`）。
...
| aria2c 工作集内存 (RSS) | 29.96 MiB | 进程物理内存常驻集 |
| aria2c 私有提交内存 (Private Memory) | 17.96 MiB | 专用内存分配 |
| aria2c 线程数 | 1 | 活动线程数 |

## 4. 结论与阶段退出判定

1. 真实 RPC 冷启动时间均值在 283.4ms 级别，远优于 500ms 试验目标，空任务与恢复任务无异常延迟。
2. 单命令 RPC 往返 P95 稳定在 1.09ms，为阶段 1 原生探针和阶段 4 原生实现确定了严格的性能对比基准。
