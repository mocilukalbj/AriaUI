...
日期 / 应用 commit / aria2 commit / 构建模式：
  2026-09-05 08:32:40 UTC / git-master / aria2 1.37.0 / .NET 10.0 (Release/Debug)
...
|---|---|---|---|---|---|---|---|
| 空任务 Session 启动 | 30 | 243.82 | 254.35 | 285.96 | 302.21 | 259.82 | < 500ms 达成 |
| 预置 10 任务 Session 恢复 | 30 | 238.94 | 247.47 | 274.23 | 342.19 | 252.74 | < 500ms 达成 |
...
|---|---|---|---|---|---|---|---|
| 第 1 轮 | aria2.getGlobalStat | 1000 | 0.39 | 0.88 | 1.17 | 1.87 | 0.90 |
| 第 2 轮 | aria2.getGlobalStat | 1000 | 0.34 | 0.80 | 1.04 | 4.21 | 0.82 |
| 第 3 轮 | aria2.getGlobalStat | 1000 | 0.14 | 0.77 | 0.99 | 3.35 | 0.77 |

> **三轮 P95 中位门禁基线值**：`1.04 ms`（阶段 4 候选原生实现上限阈值为：`1.15 ms`）。
...
| aria2c 工作集内存 (RSS) | 30.04 MiB | 进程物理内存常驻集 |
| aria2c 私有提交内存 (Private Memory) | 17.96 MiB | 专用内存分配 |
| aria2c 线程数 | 1 | 活动线程数 |

## 4. 结论与阶段退出判定

1. 真实 RPC 冷启动时间均值在 259.8ms 级别，远优于 500ms 试验目标，空任务与恢复任务无异常延迟。
2. 单命令 RPC 往返 P95 稳定在 1.04ms，为阶段 1 原生探针和阶段 4 原生实现确定了严格的性能对比基准。
