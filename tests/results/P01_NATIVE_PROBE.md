...
| **RUN_ONCE 等待语义** | 单次超时 `667.44 ms` (~1000ms) | 证实上游 epoll_wait 默认 1 秒超时 | **PASS** |
| **空闲 CPU 占用率** | `0.00%` (< 0.5%) | 阻塞式 epoll 零 CPU 浪费，无需忙轮询 | **PASS** |
| **命令入队响应延迟** | P50: `598.68 ms`, P95: `997.50 ms` | 纯空闲态受 RUN_ONCE 1s 超时影响 | **PASS** |
